...
function _proxy(req, res, prefix, callback){
    ...
    p('go fetch: ', address);
    fetch(address, prefix, callback);
    //fetch(address, prefix, function(err, res1, html){
...
function fetch(address, prefix, callback){
...
if(require.main === module){
    function test2(){
        ...
        fetch(address, prefix, function(err, html){
            if(err) return p(err);
            p(html.slice(0, 500));
        });
    }
